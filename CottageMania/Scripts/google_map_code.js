...
function setMarkers(map, locations) {
   ...
   for (var i = 0; i < locations.length; i++) {
        ...
        var marker = new google.maps.Marker({
            ...
            jQuery('#advanced_search_map_form').hide();
            if(this.image===''){
                 info_image='<img src="../Assets/' + googlecode_regular_vars.path + '/idxdefault.jpg" alt="image" />';
             }else{
                 info_image=this.image;
             }
            
            var type    =   this.type.replace(/-/g,' ');
            var type2   =   this.type2.replace(/-/g,' ');
            var in_type =   " in ";
            if(type==='' || type2===''){
                in_type=" ";
            }
            ...
            infoBox.setContent('<div class="info_details '+extra_adv_class+' "><span id="infocloser" onClick=\'javascript:infoBox.close();\' ></span><a href="../Assets/'+this.link+'">'+info_image+'</a><a href="../Assets/'+this.link+'" id="infobox_title">'+this.title+'</a><div class="prop_details"><span id="info_inside">'+type+in_type+type2+this.price+'</span></div>' );
